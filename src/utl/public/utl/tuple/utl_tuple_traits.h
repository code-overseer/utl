...
struct get_cpo_t {
    ...
    template <typename T>
    UTL_ATTRIBUTES(NODISCARD, FLATTEN) constexpr enable_if_t<sizeof(get<I>(declval<T>())) && !is_tuple<T>::value,
        result_t<I, T>>
    ...
    template <typename T>
    UTL_ATTRIBUTES(NODISCARD, FLATTEN, CONST) constexpr enable_if_t<sizeof(get<I>(declval<T>())) && is_tuple<T>::value,
        result_t<I, T>>
...
template <size_t I, typename T>
UTL_ATTRIBUTES(NODISCARD, FLATTEN) constexpr enable_if_t<!is_tuple<T>::value, result_t<I, T>> get(
    T&& t UTL_ATTRIBUTES(LIFETIMEBOUND)) noexcept(noexcept(details::get_cpo_t<I>()(declval<T>()))) {
    return details::get_cpo_t<I>()(forward<T>());
}

template <size_t I, typename T>
UTL_ATTRIBUTES(NODISCARD, FLATTEN, CONST) constexpr enable_if_t<is_tuple<T>::value, result_t<I, T>> get(
...
Target<decltype(UTL_SCOPE tuple_traits::decl_element<Is, T>())...> rebuild_target_references(
    T&& gettable, index_sequence<Is...>) noexcept;

template <template <typename...> class Target, typename T, size_t... Is>
Target<typename UTL_SCOPE tuple_element<Is, T>::type...> rebuild_target_elements(
    T&& gettable, index_sequence<Is...>) noexcept;
...
using rebind_references_t = decltype(tuple_traits::details::rebuild_target_references<Target>(
...
template <template <typename...> class Target, typename TupleLike,
    size_t N = tuple_size<TupleLike>::value>
using rebind_elements_t = decltype(tuple_traits::details::rebuild_target_elements<Target>(
    declval<TupleLike>(), make_index_sequence<N>{}));
...
auto concat_elements_helper(index_sequence<Is...>, index_sequence<Js...>) noexcept
    -> UTL_SCOPE tuple<tuple_element_t<Is, T0>..., tuple_element_t<Js, T1>...>;
template <typename T0, typename T1, size_t... Js>
auto concat_elements_helper(index_sequence<>, index_sequence<Js...>) noexcept
    -> UTL_SCOPE tuple<tuple_element_t<Js, T1>...>;

template <typename T0, typename T1, size_t... Is>
auto concat_elements_helper(index_sequence<Is...>, index_sequence<>) noexcept
...
template <typename T0>
auto concat_elements_helper() noexcept
