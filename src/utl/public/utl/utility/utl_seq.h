...
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto to_sequence(scalar_t<T, I>) noexcept
    -> sequence_t<T, I> {
    return {};
}
template <typename T, T... Is>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto to_array(sequence_t<T, Is...>) noexcept
    -> array_t<T, Is...> const& {
    return array<T, Is...>;
}
...
template <typename T, T I>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto bw_neg(scalar_t<T, I>) noexcept
    -> scalar_t<decltype(~I), ~I> {
    return {};
}
template <typename T, T... Is>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto bw_neg(sequence_t<T, Is...>) noexcept
    -> sequence_t<decltype(~declval<T>()), (~Is)...> {
    return {};
}
template <typename T, T I>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto operator~(scalar_t<T, I>) noexcept
    -> scalar_t<decltype(~I), ~I> {
    return {};
}
template <typename T, T... Is>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto operator~(sequence_t<T, Is...>) noexcept
    -> sequence_t<decltype(~declval<T>()), (~Is)...> {
    return {};
}
...
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr T access(sequence_t<T, Is...>) noexcept {
    return array<T, Is...>[N];
}

template <typename T, T... Is, size_t... Js>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto shuffle(sequence_t<T, Is...>,
    sequence_t<size_t, Js...>) noexcept -> sequence_t<T, array<T, Is...>[Js]...> {
    return {};
}
template <typename T, T... Is, size_t J>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto pow(sequence_t<T, Is...>, index_t<J>) noexcept
    -> sequence_t<T, details::pow(Is, J)...> {
    return {};
}
template <typename T, T... Is, size_t... Js>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto pow(sequence_t<T, Is...>,
    sequence_t<size_t, Js...>) noexcept -> sequence_t<T, details::pow(Is, Js)...> {
    return {};
}

template <size_t N, typename T, T I>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto repeat(scalar_t<T, I>) noexcept
    -> decltype(bw_or(zero<T, N>, scalar<T, I>)) {
    return {};
}
template <size_t N, typename T, T... Is>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto repeat(sequence_t<T, Is...>) noexcept
    -> decltype(shuffle(
        sequence<T, Is...>, div(range<size_t, N * sizeof...(Is)>, index<sizeof...(Is)>))) {
    return {};
}
template <size_t N, typename T, T... Is>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto tile(sequence_t<T, Is...>) noexcept
    -> decltype(shuffle(
        sequence<T, Is...>, mod(range<size_t, N * sizeof...(Is)>, index<sizeof...(Is)>))) {
    return {};
}
template <typename T, T... Is>
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto flip(sequence_t<T, Is...>) noexcept
    -> decltype(shuffle(
...
UTL_ATTRIBUTES(NODISCARD, CONST, _HIDE_FROM_ABI) constexpr auto concat(
