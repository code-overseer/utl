...
__UTL_HIDE_FROM_ABI T impl(T l, T r) noexcept {
    ...
    return !_subborrow_u64(0, l, r, &tmp) ? UTL_SCOPE as_signed(tmp) : sat;
}
#  endif // UTL_ARCH_x86_64
...
__UTL_HIDE_FROM_ABI T impl(T l, T r) noexcept {
    ...
    T const sat = (l >> shift) ^ UTL_NUMERIC_maximum(T);
    UTL_SCOPE make_unsigned_t<T> tmp;
    return !_subborrow_u32(0, l, r, &tmp) ? UTL_SCOPE to_signed(tmp) : sat;
}
...
__UTL_HIDE_FROM_ABI T impl(T l, T r) noexcept {
    ...
    T const sat = (l >> shift) ^ UTL_NUMERIC_maximum(T);
    UTL_SCOPE make_unsigned_t<T> tmp;
    return !_subborrow_u16(0, l, r, &tmp) ? UTL_SCOPE to_signed(tmp) : sat;
}
...
__UTL_HIDE_FROM_ABI T impl(T l, T r) noexcept {
    ...
    T const sat = (l >> shift) ^ UTL_NUMERIC_maximum(T);
    UTL_SCOPE make_unsigned_t<T> tmp;
    return !_subborrow_u8(0, l, r, &tmp) ? UTL_SCOPE to_signed(tmp) : sat;
}
} // namespace runtime
} // namespace sub_sat
} // namespace details
