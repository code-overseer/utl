...
public:
     ...
     */
    __UTL_HIDE_FROM_ABI UTL_CONSTEXPR_CXX14 nonnull_ptr(T* ptr) UTL_THROWS : ptr_(ptr) {
     ...
     */
    UTL_ATTRIBUTES(_HIDE_FROM_ABI, FLATTEN, PURE) constexpr operator T*() const {
        return get();
    }
    ...
    UTL_ATTRIBUTES(_HIDE_FROM_ABI, NODISCARD, PURE) constexpr T& operator*() const noexcept { return *ptr_; }

    /**
     * Returns the underlying raw pointer.
     */
    UTL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, PURE) constexpr T* get() const noexcept {
     ...
     */
    UTL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, CONST) constexpr explicit operator bool() const noexcept {
        return true;
    }
