...
#include "utl/exceptions/utl_program_exception.h"
...
public:
    ...
    UTL_ATTRIBUTES(NODISCARD, ALWAYS_INLINE, _HIDE_FROM_ABI) inline T const& error() const& noexcept UTL_ATTRIBUTE(
        LIFETIMEBOUND) {
        return error_;
    }
    ...
    UTL_ATTRIBUTES(NODISCARD, ALWAYS_INLINE, _HIDE_FROM_ABI) inline T& error() & noexcept UTL_ATTRIBUTE(
        LIFETIMEBOUND) {
        return error_;
    }
