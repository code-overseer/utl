...
constexpr int builtin_popcount(unsigned long long x) noexcept {
    return __builtin_popcountll(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_popcount(unsigned long x) noexcept {
    return __builtin_popcountl(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_popcount(unsigned int x) noexcept {
    return __builtin_popcount(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_popcount(unsigned short x) noexcept {
    return __builtin_popcount(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_popcount(unsigned char x) noexcept {
    return __builtin_popcount(x);
}
...
template <typename T>
UTL_CONSTEVAL int popcount(T x, int c = 0) noexcept {
...
constexpr UTL_ENABLE_IF_CXX11(bool, is_bit_readable<T>::value) popcount(T x) noexcept {
