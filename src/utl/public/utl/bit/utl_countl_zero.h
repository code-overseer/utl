...
namespace runtime {
UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_clz(unsigned long long x, unsigned long index = 0) noexcept {
    return _BitScanReverse64(&index, x) ? index : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_clz(unsigned long x, unsigned long index = 0) noexcept {
    return _BitScanReverse(&index, x) ? index : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_clz(unsigned int x, unsigned long index = 0) noexcept {
    return _BitScanReverse(&index, x) ? index : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_clz(unsigned short x, unsigned long index = 0) noexcept {
    return _BitScanReverse(&index, x) ? index - unsigned_width_diff<unsigned short>()
                                      : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_clz(unsigned char x, unsigned long index = 0) noexcept {
...
constexpr int builtin_clz(T x) noexcept {
...
UTL_ATTRIBUTES(NODISCARD, CONST)
constexpr UTL_ENABLE_IF_CXX11(int, UTL_TRAIT_VALUE(is_bit_readable, T)) countl_zero(T x) noexcept {
