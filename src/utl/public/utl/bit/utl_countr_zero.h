...
constexpr int builtin_ctz(unsigned long long x) noexcept {
    return x ? __builtin_ctzll(x) : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_ctz(unsigned long x) noexcept {
    return x ? __builtin_ctzl(x) : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_ctz(unsigned x) noexcept {
    return x ? __builtin_ctz(x) : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_ctz(unsigned short x) noexcept {
    return x ? __builtin_ctz(x) - unsigned_width_diff<unsigned short>() : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
constexpr int builtin_ctz(unsigned char x) noexcept {
...
namespace runtime {
UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_ctz(unsigned long long x, unsigned long index = 0) noexcept {
    return _BitScanForward64(&index, x) ? index : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_ctz(unsigned long x, unsigned long index = 0) noexcept {
    return _BitScanForward(&index, x) ? index : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_ctz(unsigned int x, unsigned long index = 0) noexcept {
    return _BitScanForward(&index, x) ? index : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_ctz(unsigned short x, unsigned long index = 0) noexcept {
    return _BitScanForward(&index, x) ? index : CHAR_BIT * sizeof(x);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE)
int builtin_ctz(unsigned char x, unsigned long index = 0) noexcept {
    return _BitScanForward(&index, x) ? index : CHAR_BIT * sizeof(x);
}
} // namespace runtime
...
constexpr int builtin_ctz(T x) noexcept {
...
UTL_ATTRIBUTES(NODISCARD, CONST)
constexpr UTL_ENABLE_IF_CXX11(int, UTL_TRAIT_VALUE(is_bit_readable, T)) countr_zero(T x) noexcept {
