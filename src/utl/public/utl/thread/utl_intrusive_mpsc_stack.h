 ...
 *
 * The implementation does no provide an API for popping a node off the list, the consumer thread
 * must consume the whole list, which constructs a forward_list from which individual nodes can be
 * popped.
 *
 * @tparam node_type - Node type that the linked list will use, node_type should expose the
 * `next_node` pointer via one of the following:
 * * Declare a public non-static `node_type*` data member named `next_node`
 * * Declare public non-static function members with the following signatures `node_type*
 * Getnext_node() const` & `node_type* Setnext_node(node_type*, node_type*)`
 * * Otherwise, the expressions `Getnext_node(node)` & `Setnext_node(node, value_to_set)` must be
 * valid through argument-dependent-lookup, where `node` and `value_to_set` are `node_type*`
 ...
 * `multi_producer_stack::PushFront`
 *  *
...
public:
    ...
    multi_producer_stack(multi_producer_stack const&) = delete;
    multi_producer_stack& operator=(multi_producer_stack const&) = delete;
    ...
    multi_producer_stack(Args&&... args) : node_policy(forward<Args>(args)...)
                                         , head_(nullptr) {}

    /**
     * Creates/retrieves a node from the provided argument(s) to be added to the head of the linked
     * list
     *
    ...
    node_pointer pop_front() {
        ...
        if (output == nullptr) {
            return node_pointer(nullptr, static_cast<node_policy const&>(*this));
        }
        ...
        do {
            ...
            output, new_head, std::memory_order_acquire, std::memory_order_release));

        return node_pointer(new_head, static_cast<node_policy const&>(*this));
    }
    ...
    /**
     * Creates a consumer list which atomically takes ownership of the whole list and clears the
    ...
    [[nodiscard]] consume_list consume() {
        return consume_list(head_, static_cast<node_policy const&>(*this));
    }
    ...
    /**
     * Atomically clears the list, and subsequently (non-atomically) releasing ownership of every
     * node
     */
    void clear() noexcept(noexcept(next((node_type*)0), next((node_type*)0, (node_type*)0),
        node_policy::release((node_type*)0))) {
        node_type* current_node =
            UTL_ATOMIC_EXCHANGE(addressof(head_), nullptr, memory_order_acquire);
    ...
    [[nodiscard]] bool empty() const noexcept {
        return UTL_ATOMIC_LOAD(addressof(head_), memory_order_relaxed) == nullptr;
    }
...
private:
    void push_front_impl(node_type* node) {
        ...
        // The memory operations on every node must be visible; if this was relaxed, only the memory
        // operation on the head node may be visible on the consumer thread, which may result in
        // invalid data access when the consumer thread accesses nodes below `head_`
        node_type* current_head = UTL_ATOMIC_LOAD(addressof(head_), memory_order_acquire);
        do {
            set_next(node, current_head);
        } while (
            !UTL_ATOMIC_CAS(addressof(head_), node, memory_order_release, memory_order_acquire));
    }
