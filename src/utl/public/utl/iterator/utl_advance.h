...
template <class It>
UTL_CONSTEXPR_CXX14 void forward(It& it, difference_t<It> n, UTL_SCOPE random_access_iterator_tag) noexcept(
    noexcept(it += n)) {
    it += n;
}

template <typename It>
UTL_CONSTEXPR_CXX14 void backward(It& it, difference_t<It> n, UTL_SCOPE bidirectional_iterator_tag) noexcept(
    noexcept(++n, --it)) {
    do {
        ++n;
        --it;
...
template <typename It>
UTL_CONSTEXPR_CXX14 void backward(It& it, difference_t<It> n, UTL_SCOPE random_access_iterator_tag) noexcept(
    noexcept(it += n)) {
    it += n;
}

template <typename It>
UTL_CONSTEXPR_CXX14 void impl(It& it, difference_t<It> n, UTL_SCOPE input_iterator_tag t) noexcept(
...
UTL_CONSTEXPR_CXX14 auto impl(It& it, difference_t<It> n, Tag t) noexcept(
...
template <typename It>
using is_nothrow = decltype(UTL_SCOPE details::advance::nothrow_check<It>(0));
...
UTL_CONSTEXPR_CXX14 auto advance(It& it, Distance n) noexcept(
