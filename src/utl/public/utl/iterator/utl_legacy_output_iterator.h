...
struct is_legacy_output_iterator :
    UTL_SCOPE bool_constant<legacy_output_iterator<It, ValueType>> {};
...
template <typename It, typename ValueType>
auto check(int) noexcept -> UTL_SCOPE conjunction<UTL_SCOPE is_legacy_iterator<It>,
...
template <typename It, typename ValueType>
using implemented = decltype(UTL_SCOPE details::legacy_output_iterator::check<It, ValueType>(0));
