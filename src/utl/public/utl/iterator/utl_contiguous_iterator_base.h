...
private:
    ...
    UTL_ITERATOR_PURE UTL_HIDE_FROM_ABI static constexpr value_type* get_ptr(
...
public:
    UTL_ITERATOR_PURE UTL_HIDE_FROM_ABI constexpr value_type& operator*() const noexcept {
        return *ptr_;
    }
    ...
    UTL_ITERATOR_CONST UTL_HIDE_FROM_ABI friend constexpr It operator+(
    ...
    UTL_ITERATOR_CONST UTL_HIDE_FROM_ABI friend constexpr difference_type operator-(
        It left, It right) noexcept {
        return get_ptr(left) - get_ptr(right);
    }

    UTL_HIDE_FROM_ABI friend constexpr It& operator+=(It& it, difference_type offset) noexcept {
        return set_ptr(it, get_ptr(it) + offset), it;
    }
    ...
    UTL_HIDE_FROM_ABI friend constexpr It& operator++(It& it) noexcept {
        return set_ptr(it, get_ptr(it) + 1), it;
    }

    UTL_HIDE_FROM_ABI friend UTL_CONSTEXPR_CXX14 It operator++(It& it, int) noexcept {
        It before = it;
        ++it;
        return before;
    }
    ...
    UTL_ITERATOR_PURE UTL_HIDE_FROM_ABI constexpr value_type& operator[](
        difference_type offset) const noexcept {
        return *(ptr_ + offset);
    }

    UTL_ITERATOR_PURE UTL_HIDE_FROM_ABI friend constexpr bool operator==(
...
#ifdef UTL_CXX20
    UTL_ITERATOR_PURE UTL_HIDE_FROM_ABI friend constexpr auto operator<=>(
        It const& left, It const& right) noexcept {
        return get_ptr(left) <=> get_ptr(right);
    }
#endif
