...
struct UTL_PUBLIC_TEMPLATE char_traits {
    ...
    UTL_ATTRIBUTES(NODISCARD, PURE) UTL_HIDE_FROM_ABI static constexpr int compare(
        char_type const* lhs, char_type const* rhs, size_t length) noexcept {
        return libc::strncmp(lhs, rhs, libc::element_count_t(length));
    }

    UTL_ATTRIBUTES(NODISCARD, PURE) UTL_HIDE_FROM_ABI static constexpr size_t length(
        char_type const* str) noexcept {
        return libc::strlen(str);
    }

    UTL_ATTRIBUTES(NODISCARD, PURE) UTL_HIDE_FROM_ABI static constexpr char_type const* find(
        char_type const* str, size_t length, char_type const ch) noexcept {
        return libc::strnchr(str, ch, libc::element_count_t(length));
    }

    UTL_HIDE_FROM_ABI static constexpr char_type* move(
        char_type* s1, char_type const* s2, size_t length) noexcept {
        return libc::memmove(s1, s2, libc::element_count_t(length));
    }

    UTL_HIDE_FROM_ABI static constexpr char_type* copy(
    ...
    UTL_HIDE_FROM_ABI static constexpr char_type* assign(
        char_type* str, size_t length, char_type ch) noexcept {
        return libc::strnset(str, ch, libc::element_count_t(length));
    }

    UTL_ATTRIBUTES(NODISCARD, CONST) UTL_HIDE_FROM_ABI static constexpr int_type not_eof(int_type c) noexcept {
        return eq_int_type(c, eof()) ? ~eof() : c;
    }
    UTL_ATTRIBUTES(NODISCARD, CONST) UTL_HIDE_FROM_ABI static constexpr char_type to_char_type(
        int_type c) noexcept {
        return char_type(c);
    }
    UTL_ATTRIBUTES(NODISCARD, CONST) UTL_HIDE_FROM_ABI static constexpr int_type to_int_type(
        char_type c) noexcept {
        return int_type(as_unsigned(c));
    }
    UTL_ATTRIBUTES(NODISCARD, CONST) UTL_HIDE_FROM_ABI static constexpr bool eq_int_type(
        int_type c1, int_type c2) noexcept {
        return c1 == c2;
    }
