...
struct UTL_PUBLIC_TEMPLATE char_traits {
    ...
    UTL_ATTRIBUTES(HIDE_FROM_ABI, ALWAYS_INLINE) static constexpr char_type* assign(
        char_type* str, size_t length, char_type ch) noexcept {
        return libc::strnset(str, ch, libc::element_count_t(length));
    }

    UTL_ATTRIBUTE(CONST_API) static constexpr int_type not_eof(int_type c) noexcept {
        return eq_int_type(c, eof()) ? ~eof() : c;
    }
    UTL_ATTRIBUTE(CONST_API) static constexpr char_type to_char_type(int_type c) noexcept {
        return char_type(c);
    }
    UTL_ATTRIBUTE(CONST_API) static constexpr int_type to_int_type(char_type c) noexcept {
        return int_type(as_unsigned(c));
    }
    UTL_ATTRIBUTE(CONST_API) static constexpr bool eq_int_type(int_type c1, int_type c2) noexcept {
        return c1 == c2;
    }
    UTL_ATTRIBUTE(CONST_API) static constexpr int_type eof() noexcept { return int_type(EOF); }
