...
UTL_CONSTEVAL T* memchr(T const* str, U value, size_t count) noexcept {
    return count == 0   ? nullptr
        : *str == value ? const_cast<T*>(str)
                        : memchr(str + 1, value, count - 1);
}
...
UTL_CONSTEVAL int strncmp(char const* left, char const* right, size_t len) noexcept {
...
UTL_CONSTEVAL int strncmp(wchar_t const* left, wchar_t const* right, size_t len) noexcept {
