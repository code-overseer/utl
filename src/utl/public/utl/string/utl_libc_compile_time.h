...
template <typename T>
UTL_HIDE_FROM_ABI constexpr T* reverse_memcpy(
    T* dst, T const* src, element_count_t count, T* org) noexcept {
    return count == 0 ? org : ((*--dst = *--src), memcpy(dst, src, count - 1, org));
}

template <typename T>
UTL_HIDE_FROM_ABI constexpr T* memset(T* dst, T const src, element_count_t count, T* org) noexcept {
...
UTL_HIDE_FROM_ABI constexpr int memcmp(
...
UTL_HIDE_FROM_ABI constexpr T* memchr(T const* str, U value, size_t bytes) noexcept {
...
template <typename T>
UTL_HIDE_FROM_ABI constexpr size_t strlen(T const* str, size_t r = 0) noexcept {
    return !*str ? r : strlen(str, r + 1);
}

template <typename T>
UTL_HIDE_FROM_ABI constexpr T* strchr(T const* str, T const ch) noexcept {
...
template <typename T>
UTL_HIDE_FROM_ABI constexpr int strcmp(T const* left, T const* right) noexcept {
    return !*left && !*right ? 0
        : (*left < *right)   ? -1
        : (*right < *left)   ? 1
...
template <typename T>
UTL_HIDE_FROM_ABI constexpr T* strnchr(T const* str, T const ch, element_count_t len) noexcept {
...
template <typename T>
UTL_HIDE_FROM_ABI constexpr T* strnset(
...
UTL_HIDE_FROM_ABI constexpr T* memcpy(T* dst, T const* src, element_count_t count) noexcept {
...
UTL_HIDE_FROM_ABI constexpr size_t strlen(T const* str) noexcept {
    return recursive::strlen(str);
}
...
template <UTL_CONCEPT_CXX20(string_char) T UTL_REQUIRES_CXX11(is_string_char<T>::value)>
UTL_HIDE_FROM_ABI constexpr T* strnchr(
