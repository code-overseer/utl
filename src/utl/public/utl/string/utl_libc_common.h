...
UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE) constexpr element_count_t min(
    element_count_t l, element_count_t r) noexcept {
    return (size_t)l < (size_t)r ? l : r;
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE) constexpr element_count_t max(
    element_count_t l, element_count_t r) noexcept {
    return (size_t)l > (size_t)r ? l : r;
}

template <typename T>
UTL_LIBC_CONST UTL_HIDE_FROM_ABI constexpr size_t byte_count(element_count_t c) noexcept {
    return sizeof(T) * size_t(c);
}

template <typename T>
UTL_LIBC_CONST UTL_HIDE_FROM_ABI constexpr bool is_pointer_in_range(
    T const* begin, T const* end, T const* ptr) noexcept {
    return UTL_CONSTANT_P(begin <= end && ptr < end) || !(ptr < begin) && ptr < end;
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE) UTL_HIDE_FROM_ABI constexpr element_count_t operator-(
    element_count_t val, size_t op) noexcept {
    return (element_count_t)((size_t)val - op);
}

UTL_ATTRIBUTES(NODISCARD, CONST, ALWAYS_INLINE) constexpr bool operator==(
    element_count_t val, size_t op) noexcept {
    return (size_t)val == op;
}

template <typename T>
UTL_LIBC_CONST UTL_HIDE_FROM_ABI constexpr T* operator+(T* ptr, element_count_t offset) noexcept {
...
UTL_LIBC_CONST UTL_HIDE_FROM_ABI unsigned char as_byte(T val) noexcept {
