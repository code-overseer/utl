...
public:
    ...
    __UTL_HIDE_FROM_ABI explicit inline constexpr scope_exit(Fn&& func) noexcept(
        UTL_TRAIT_is_nothrow_constructible(F, Fn))
        : base_type(__UTL forward<Fn>(func)) {}
    scope_exit(scope_exit const&) = delete;
    __UTL_HIDE_FROM_ABI inline constexpr scope_exit(move_t&& other) noexcept(
...
template <typename Fn>
UTL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE) inline constexpr auto make_scope_exit(Fn&& f) noexcept(
