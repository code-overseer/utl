...
struct member_info<Type Class::*, V> : member_traits<Type Class::*> {
    ...
    template<typename T, typename... Args, typename = enable_if_t<
            is_base_of<Class, decay_t<T>>::value &&
            is_function<type>::value
        >
    >
    constexpr decltype((declval<copy_cvref_t<T, Class>>()().*value)(declval<Args>()...)) operator()(T&& instance, Args&&... args) const {
        return (static_cast<copy_cvref_t<T&&, Class>>(forward<T>(instance))->*value)(forward<Args>(args)...);
    }

    template<typename T, typename = enable_if_t<
            is_convertible<T&&, copy_cvref_t<T&&, Class>>::value &&
            !is_function<type>::value
        >
    >
...
#define UTL_MEMBER_INFO(CLASS, MEMBER) \
    UTL_SCOPE member_info_of<&CLASS::MEMBER>
