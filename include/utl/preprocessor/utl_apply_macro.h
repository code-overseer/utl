...
#define UTL_APPLY_MACRO_NEXT(test, next) UTL_APPLY_MACRO_NEXT1(UTL_APPLY_MACRO_GET_END test, next)
#define UTL_APPLY_MACRO0(f, x, peek, ...) f(x) UTL_APPLY_MACRO_NEXT(peek, UTL_APPLY_MACRO1)(f, peek, __VA_ARGS__)
#define UTL_APPLY_MACRO1(f, x, peek, ...) f(x) UTL_APPLY_MACRO_NEXT(peek, UTL_APPLY_MACRO0)(f, peek, __VA_ARGS__)
#   else   /* ifndef UTL_COMPILER_MSVC */
#define UTL_APPLY_MACRO_NEXT1(test, next) UTL_PASTE(UTL_APPLY_MACRO_NEXT0(test, next, 0))
#define UTL_APPLY_MACRO_NEXT(test, next) UTL_PASTE(UTL_APPLY_MACRO_NEXT1(UTL_APPLY_MACRO_GET_END test, next))
#define UTL_APPLY_MACRO0(f, x, peek, ...) f(x) UTL_PASTE(UTL_APPLY_MACRO_NEXT(peek, UTL_APPLY_MACRO1)(f, peek, __VA_ARGS__))
#define UTL_APPLY_MACRO1(f, x, peek, ...) f(x) UTL_PASTE(UTL_APPLY_MACRO_NEXT(peek, UTL_APPLY_MACRO0)(f, peek, __VA_ARGS__))
#   endif  /* ifndef UTL_COMPILER_MSVC */

#define UTL_APPLY_MACRO_LIST_NEXT1(test, next) UTL_APPLY_MACRO_NEXT0(test, UTL_APPLY_MACRO_COMMA next, 0)
#define UTL_APPLY_MACRO_LIST_NEXT(test, next)  UTL_APPLY_MACRO_LIST_NEXT1(UTL_APPLY_MACRO_GET_END test, next)

#define UTL_APPLY_MACRO_LIST0(f, x, peek, ...) f(x) UTL_PASTE(UTL_APPLY_MACRO_LIST_NEXT(peek, UTL_APPLY_MACRO_LIST1)(f, peek, __VA_ARGS__))
#define UTL_APPLY_MACRO_LIST1(f, x, peek, ...) f(x) UTL_PASTE(UTL_APPLY_MACRO_LIST_NEXT(peek, UTL_APPLY_MACRO_LIST0)(f, peek, __VA_ARGS__))

/**
 * Applies the function macro `f` to each of the remaining parameters.
 */
#define UTL_APPLY_MACRO(f, ...) UTL_EVAL(UTL_APPLY_MACRO1(f, __VA_ARGS__, ()()(), ()()(), ()()(), 0))
