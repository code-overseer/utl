...
private:
    ...
    template <typename U0, typename U1>
    UTL_CONSTEXPR_CXX14 pair const& assign(U0&& other_first, U1&& other_second) const
...
public:
    template <typename U0 = T0, typename U1 = T1>
    UTL_CONSTEXPR_CXX14 enable_if_t<traits::template is_swappable_with<U0&, U1&>::value> swap(
        pair<U0, U1>& other) noexcept(traits::template is_nothrow_swappable_with<U0&, U1&>::value) {
    ...
    template <typename U0 = T0, typename U1 = T1>
    UTL_CONSTEXPR_CXX14 enable_if_t<traits::template is_const_swappable_with<U0&, U1&>::value> swap(
        pair<U0, U1>& other) const
...
public:
    template <typename U0 = T0, typename U1 = T1,
        enable_if_t<traits::template is_implicit_constructible<U0 const&, U1 const&>::value, bool> =
            true>
    constexpr pair(T0 const& t0, T1 const& t1) noexcept(traits::is_nothrow_copy_constructible)
        : first(t0)
        , second(t1) {}
...
#  define UTL_DEFINE_PAIR_PIECEWISE_CONSTRUCTOR()                                                 \
      ...
      constexpr pair<T0, T1>::pair(piecewise_construct_t, tuple<Args0...> l0,                     \
...
template <typename T, typename U>
constexpr pair<unwrap_reference_t<decay_t<T>>, unwrap_reference_t<decay_t<U>>> make_pair(
    T&& t, U&& u) {
...
template <size_t I, typename T0, typename T1>
struct tuple_element<I, UTL_SCOPE pair<T0, T1>> :
