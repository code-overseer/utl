...
requires (... && requires {
    typename UTL_SCOPE common_type<Ts, Us>::type;
})
...
public:
    ...
    constexpr storage& operator=(storage const&) noexcept(traits::is_nothrow_copy_assignable) = default;

#ifdef UTL_ENFORCE_NONMOVABILIITY
    constexpr storage(move_construct_t&&) noexcept(traits::is_nothrow_move_constructible) = delete;
    constexpr storage& operator=(move_assign_t&&) noexcept(traits::is_nothrow_move_assignable) = delete;
#else
    ...
    template<typename Alloc, typename U> requires
        UTL_SCOPE uses_allocator<T, Alloc>::value &&
    ...
    template<typename Alloc, typename U> requires
        UTL_SCOPE uses_allocator<T, Alloc>::value &&
        (!UTL_SCOPE is_constructible<T, allocator_arg_t, Alloc const&, U>::value) &&
        UTL_SCOPE is_constructible<T, Alloc const&, U>::value
    constexpr storage(allocator_arg_t, Alloc const& alloc, U&& other_head) noexcept(
        requires(U&& u, Alloc const& a) {
    ...
    template<typename U>
    constexpr void swap(storage<U>& other) const noexcept(
        ...
        template<typename U>
    constexpr void swap(storage<U> const& other) const noexcept(
    ...
    template<typename U>
    constexpr storage& assign(U&& other) noexcept(
        requires(T& h, U&& u) {
...
struct storage<T, Tail...> : variadic_traits<T, Tail...> {
    ...
    template<typename UHead, typename... UTail> requires
    ...
    template<typename Alloc, typename UHead, typename... UTail> requires
        UTL_SCOPE uses_allocator<T, Alloc>::value &&
    ...
    constexpr storage(allocator_arg_t, Alloc const& alloc, UHead&& other_head, UTail&&... other_tail) noexcept(
        requires(Alloc const& a, UHead&& h, UTail&&... ts) {
    ...
    template<typename... Us>
    constexpr void swap(storage<Us...> const& other) const noexcept(
    ...
    template<typename... Us>
    constexpr void swap(storage<Us...>& other) const noexcept(
    ...
    constexpr storage const& assign(UHead&& other_head, UTail&&... other_tail) const noexcept(
        requires(T const& h, tail_type const& tail, UHead&& u, UTail&&... ts) {
            { h = forward<UHead>(u) } noexcept;
            { tail.assign(forward<UTail>(ts)...) } noexcept;
        }
    ) {
    ...
    template<size_t I> requires (I == 0)
    UTL_ATTRIBUTES(NODISCARD, CONST) constexpr auto get() const && noexcept UTL_ATTRIBUTE(LIFETIMEBOUND) -> T const&& {
        return move(head);
    }
    ...
    template<size_t I> requires (I > 0) && (I < traits::size)
    UTL_ATTRIBUTES(NODISCARD, CONST, FLATTEN) constexpr auto get() && noexcept UTL_ATTRIBUTE(LIFETIMEBOUND) -> template_element_t<I, storage>&& {
    ...
    template<size_t I> requires (I > 0) && (I < traits::size)
    UTL_ATTRIBUTES(NODISCARD, CONST, FLATTEN) constexpr auto get() const && noexcept UTL_ATTRIBUTE(LIFETIMEBOUND) -> template_element_t<I, storage> const&& {
        return move(tail).template get<I - 1>();
    }
...
private:
    ...
    constexpr tuple(allocator_arg_t, Alloc const& alloc, TupleLike&& other, index_sequence<Is...>) noexcept(
        requires(Alloc const& a, TupleLike&& t) {
    ...
    constexpr tuple const& assign(TupleLike&& other, index_sequence<Is...>) const noexcept(
        requires(base_type const& b, TupleLike&& t) {
            { b.assign(TT_SCOPE get<Is>(forward<TupleLike>(t))...) } noexcept;
        }
    ) {
...
public:
    ...
    constexpr tuple const& operator=(U const& other) const noexcept(noexcept(assign(other, index_sequence_for<Types...>{}))) {
        return assign(other, index_sequence_for<Types...>{});
    }

    template<same_as<tuple> U = tuple>
    constexpr tuple const& operator=(U&& other) const noexcept(noexcept(assign(declval<U>(), index_sequence_for<Types...>{}))) {
        return assign(move(other), index_sequence_for<Types...>{});
    }

public:
    template<same_as<Types>... Us> requires
        traits::template is_swappable_with<Us&...>::value
    ...
    template<same_as<Types>... Us> requires
        traits::template is_const_swappable_with<Us const&...>::value
    constexpr void swap(tuple<Us...> const& other) const noexcept(
        traits::template is_nothrow_const_swappable_with<Us const&...>::value
    ) {
        static_assert(is_base_of<details::tuple::storage<Us...>, tuple<Us...>>::value, "tuple must inherit from storage");
    ...
    friend inline constexpr void swap(swap_type& l, swap_type& r) noexcept(traits::is_nothrow_swappable) {
        l.swap(r);
    }

    friend inline constexpr void swap(const_swap_type& l, const_swap_type& r) noexcept(traits::is_nothrow_const_swappable) {
        l.swap(r);
    }

public:
    explicit(traits::is_explicit_default_constructible)
    constexpr tuple() noexcept(utility::conjunction_sequence<sizeof...(Types)>([](auto i) {
        return requires {
...
public:
    explicit(traits::template is_explicit_constructible<Types const&...>::value)
    constexpr tuple(Types const&... args) noexcept(traits::is_nothrow_copy_constructible) : base_type(args...) {}

public:
    template<typename UHead, typename... UTail> requires
        (sizeof...(Types) == sizeof...(UTail) + 1) &&
        traits::template is_constructible<UHead, UTail...>::value &&
        (!traits::template is_dangling<UHead&&, UTail&&...>::value)
    explicit(traits::template is_explicit_constructible<UHead, UTail...>::value)
    constexpr tuple(UHead&& head, UTail&&... tail) noexcept(
        requires(UHead&& head, UTail&&... tail) {
...
public:
    template<typename... UTypes> requires
        (sizeof...(Types) == sizeof...(UTypes)) &&
        traits::template is_constructible<UTypes&...>::value &&
        (!traits::template is_dangling<UTypes&...>::value)
    explicit(traits::template is_explicit_constructible<UTypes&...>::value)
...
public:
    template<typename... UTypes> requires
        ...
        (!traits::template is_dangling<UTypes const&...>::value)
    explicit(traits::template is_explicit_constructible<UTypes const&...>::value)
...
public:
    template<tuple_like TupleLike> requires requires {
    ...
    explicit(TT_SCOPE rebind_references_t<traits::template is_explicit_constructible, TupleLike, sizeof...(Types)>::value)
    ...
    template<tuple_like TupleLike> requires requires {
        ...
        requires TT_SCOPE rebind_references_t<
                traits::template is_constructible,
                TupleLike
            >::value;
        requires TT_SCOPE rebind_references_t<
                traits::template is_dangling,
                TupleLike
            >::value;
    }
    explicit(TT_SCOPE rebind_references_t<traits::template is_explicit_constructible, TupleLike, sizeof...(Types)>::value)
...
public:
    template<typename Alloc> requires
        traits::template is_constructible_with_allocator<Alloc>::value
    explicit(traits::template is_explicit_constructible_with_allocator<Alloc>::value)
...
public:
    ...
    explicit(traits::template is_explicit_constructible_with_allocator<Alloc, Types const&>::value)
...
public:
    template<typename Alloc, typename... UTypes> requires
        (sizeof...(Types) == sizeof...(UTypes)) &&
        ...
        (!traits::template is_dangling_without_allocator<Alloc, UTypes...>::value)
    explicit constexpr tuple(allocator_arg_t, Alloc const& alloc, UTypes&&... args) noexcept(
        requires(Alloc const& alloc, UTypes&&... args) {
...
public:
    template<typename Alloc, typename... UTypes> requires
        ...
        traits::template is_constructible_with_allocator<Alloc, UTypes&...>::value &&
        (!traits::template is_dangling_without_allocator<Alloc, UTypes&...>::value)
    explicit(traits::template is_explicit_constructible_with_allocator<Alloc, UTypes&...>::value)
    constexpr tuple(allocator_arg_t, Alloc const& alloc, tuple<UTypes...>& other) noexcept(
        requires(Alloc const& alloc, tuple<UTypes...>& other) {
...
public:
    template<typename Alloc, typename... UTypes> requires
        ...
        (!traits::template is_dangling_without_allocator<Alloc, UTypes const&...>::value)
    explicit(traits::template is_explicit_constructible_with_allocator<Alloc, UTypes const&...>::value)
    constexpr tuple(allocator_arg_t, Alloc const& alloc, tuple<UTypes...> const& other) noexcept(
        requires(Alloc const& alloc, tuple<UTypes...> const& other) {
            { tuple(allocator_arg, alloc, other, index_sequence_for<UTypes...>{}) } noexcept;
        }
    ) : tuple(allocator_arg, alloc, other, index_sequence_for<UTypes...>{}) {}
...
public:
    template<typename Alloc, tuple_like TupleLike> requires requires {
        requires tuple_size<TupleLike>::value == sizeof...(Types);
        requires !same_as<remove_cvref_t<TupleLike>, tuple>;
        requires TT_SCOPE rebind_references_t<
                variadic_proxy<
                    traits::template is_constructible_with_allocator,
                    Alloc
                >::template apply,
                TupleLike
            >::value;
    ...
    explicit(
        ...
        requires(Alloc const& alloc, TupleLike&& other) {
            { tuple(allocator_arg, alloc, forward<TupleLike>(other), index_sequence_for<Types...>{}) } noexcept;
        }
...
public:
    ...
    constexpr tuple(allocator_arg_t, Alloc const& alloc, tuple const& other) noexcept(
        requires(Alloc const& alloc, tuple const& other) {
    ...
    constexpr tuple(allocator_arg_t, Alloc const& alloc, tuple&& other) noexcept(
        requires(Alloc const& alloc, tuple&& other) {
...
public:
    template<typename... UTypes> requires
        traits::template is_assignable<UTypes const&...>::value
    constexpr tuple& operator=(tuple<UTypes...> const& other) noexcept(
        requires(tuple& t, tuple<UTypes...> const& other) {
    ...
    template<assignable_from<Types const&>... UTypes> requires
        traits::template is_const_assignable<UTypes const&...>::value
    constexpr tuple const& operator=(tuple<UTypes...> const& other) const noexcept(
        requires(tuple const& t, tuple<UTypes...> const& other) {
            { t.assign(other, index_sequence_for<Types...>{}) } noexcept;
        }
    ) {
        return assign(other, index_sequence_for<Types...>{});
...
public:
    template<typename... UTypes> requires
        traits::template is_assignable<UTypes&...>::value
    constexpr tuple& operator=(tuple<UTypes...>& other) noexcept(
    ...
    template<assignable_from<Types&>... UTypes> requires
        traits::template is_const_assignable<UTypes&...>::value
    constexpr tuple const& operator=(tuple<UTypes...>& other) const noexcept(
        requires(tuple const& t, tuple<UTypes...>& other) {
            { t.assign(other, index_sequence_for<Types...>{}) } noexcept;
        }
    ) {
        return assign(other, index_sequence_for<Types...>{});
...
public:
    template<tuple_like TupleLike> requires requires {
        requires tuple_size<TupleLike>::value == sizeof...(Types);
        requires !same_as<remove_cvref_t<TupleLike>, tuple>;
        requires TT_SCOPE rebind_references_t<traits::template is_assignable, TupleLike, sizeof...(Types)>::value;
    }
    constexpr tuple& operator=(TupleLike&& other) noexcept(
        requires(tuple& t, TupleLike&& other) {
    ...
    template<tuple_like TupleLike> requires requires {
        requires tuple_size<TupleLike>::value == sizeof...(Types);
        requires !same_as<remove_cvref_t<TupleLike>, tuple>;
        requires TT_SCOPE rebind_references_t<traits::template is_const_assignable, TupleLike, sizeof...(Types)>::value;
    }
    constexpr tuple const& operator=(TupleLike&& other) const noexcept(
        requires(tuple const& t, TupleLike&& other) {
            { t.assign(forward<TupleLike>(other), index_sequence_for<Types...>{}) } noexcept;
        }
    ) {
        return assign(forward<TupleLike>(other), index_sequence_for<Types...>{});
