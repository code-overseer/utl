...
struct is_all_reference_impl<T, index_sequence<Is...>> : conjunction<
    is_reference<tuple_element_t<Is, T>>...
> {};
template<typename T>
...
constexpr auto tuple_cat_impl_idx(T0&& tuple0, T1&& tuple1, index_sequence<I0...>, index_sequence<I1...>) noexcept(
    TT_SCOPE is_all_nothrow_gettable<T0>::value &&
    TT_SCOPE is_all_nothrow_gettable<T1>::value &&
    is_nothrow_constructible<
        TT_SCOPE concat_elements_t<T0, T1>,
        decltype(TT_SCOPE get<I0>(declval<T0>()))...,
        decltype(TT_SCOPE get<I1>(declval<T1>()))...
    >::value
) -> TT_SCOPE concat_elements_t<T0, T1> {
...
constexpr auto tuple_cat_impl(T0&& t0, T1&& t1) noexcept(
    noexcept(tuple_cat_impl_idx(declval<T0>(), declval<T1>(), tuple_index_sequence<T0>{}, tuple_index_sequence<T1>{}))
) -> TT_SCOPE concat_elements_t<T0, T1> {
    static_assert(conjunction<
            is_all_reference<T0>,
            is_all_reference<T1>
        >::value, "Elements must all be references");
    return tuple_cat_impl_idx(
        forward<T0>(t0),
        forward<T1>(t1),
        tuple_index_sequence<T0>{},
        tuple_index_sequence<T1>{});
}
...
constexpr auto tuple_cat_impl(T0&& t0, T1&& t1, T2&& t2, Tail&&... tail) noexcept(
    noexcept(
        tuple_cat_impl(
            tuple_cat_impl(declval<T0>(), declval<T1>()),
            declval<T2>(), declval<Tail>()...
        )
    )
) -> TT_SCOPE concat_elements_t<T0, T1, T2, Tail...> {
    static_assert(conjunction<
            is_all_reference<T0>,
            is_all_reference<T1>,
            is_all_reference<T2>,
            is_all_reference<Tail>...
        >::value, "Elements must all be references");
    return tuple_cat_impl(tuple_cat_impl(forward<T0>(t0), forward<T1>(t1)), forward<T2>(t2), forward<Tail>(tail)...);
}
...
constexpr auto tuple_cat(Tuples&&... args) noexcept(
    ...
    return return_type(details::tuple::tuple_cat_impl(details::tuple::tuple_forward(forward<Tuples>(args))...));
}
