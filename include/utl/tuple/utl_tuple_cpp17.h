...
struct common_type_impl<UTL_SCOPE tuple<Ts...>, UTL_SCOPE tuple<Us...>, void_t<common_type_t<Ts, Us>...>> {
    using type = UTL_SCOPE tuple<common_type_t<Ts, Us>...>;
};

template<typename, typename, template<typename> class TQual, template<typename> class UQual, typename = void>
struct basic_ref_impl {};

template<typename... Ts, typename... Us, template<typename> class TQual, template<typename> class UQual>
...
struct basic_common_reference<UTL_SCOPE tuple<Ts...>, UTL_SCOPE tuple<Us...>, TQual, UQual> :
    UTL_SCOPE details::tuple::basic_ref_impl<UTL_SCOPE tuple<Ts...>, UTL_SCOPE tuple<Us...>, TQual, UQual> {};
}
...
public:
    ...
    using head_type = T;
    using move_assign_t = conditional_t<traits::is_move_assignable,
        invalid_t,
        storage>;
    using move_construct_t = conditional_t<traits::is_move_constructible,
        invalid_t,
        storage>;
    ...
    UTL_CONSTEXPR_CXX14 storage& operator=(storage const&) noexcept(traits::is_nothrow_copy_assignable) = default;

#ifdef UTL_ENFORCE_NONMOVABILIITY
    constexpr storage(move_construct_t&&) noexcept(traits::is_nothrow_move_constructible) = delete;
    UTL_CONSTEXPR_CXX14 storage& operator=(move_assign_t&&) noexcept(traits::is_nothrow_move_assignable) = delete;
#else
    ...
    template<typename U, enable_if_t<
        !is_same<U, storage>::value,
    int> = 1>
    constexpr storage(U&& head) noexcept(traits::template is_nothrow_constructible<U>::value) : head(forward<U>(head)) {}

    template<typename Alloc, enable_if_t<
        conjunction<
            UTL_SCOPE uses_allocator<T, Alloc>,
            UTL_SCOPE is_constructible<T, allocator_arg_t, Alloc const&>
        >::value,
    int> = 0>
    ...
    template<typename Alloc, enable_if_t<
        conjunction<
            ...
            UTL_SCOPE is_constructible<T, Alloc const&>
        >::value,
    int> = 1>
    constexpr storage(allocator_arg_t, Alloc const& alloc) noexcept(
    ...
    template<typename Alloc, enable_if_t<
        !UTL_SCOPE uses_allocator<T, Alloc>::value,
    int> = 2>
    constexpr storage(allocator_arg_t, Alloc const& alloc) noexcept(
        UTL_SCOPE is_nothrow_constructible<T>::value
    ) : head() {}

    template<typename Alloc, typename U, enable_if_t<
        conjunction<
            UTL_SCOPE uses_allocator<T, Alloc>,
            UTL_SCOPE is_constructible<T, allocator_arg_t, Alloc const&, U>
        >::value,
    int> = 0>
    ...
    template<typename Alloc, typename U, enable_if_t<
        conjunction<
            ...
            UTL_SCOPE is_constructible<T, U, Alloc const&>
        >::value,
    int> = 1>
    constexpr storage(allocator_arg_t, Alloc const& alloc, U&& other_head) noexcept(
    ...
    template<typename U = T>
    UTL_CONSTEXPR_CXX14 void swap(storage<U>& other) const noexcept(
        traits::template is_nothrow_const_swappable_with<U&>::value
    ) {
        UTL_SCOPE utility::swap(head, other.head);
    }

    template<typename U = T>
    UTL_CONSTEXPR_CXX14 void swap(storage<U> const& other) const noexcept(
        traits::template is_nothrow_const_swappable_with<U const&>::value
    ) {
        UTL_SCOPE utility::swap(head, other.head);
    }
    ...
    template<typename U>
    constexpr storage const& assign(U&& other) const noexcept(
        traits::template is_nothrow_const_assignable<U>::value
    ) {
        head = forward<U>(other);
        return *this;
    }
    ...
    template<size_t I>
    UTL_ATTRIBUTES(NODISCARD, CONST) UTL_CONSTEXPR_CXX14 auto get() && noexcept UTL_ATTRIBUTE(LIFETIMEBOUND) -> enable_if_t<!I, T&&> {
        return move(head);
    }
    ...
    template<size_t I>
    UTL_ATTRIBUTES(NODISCARD, CONST) constexpr auto get() const && noexcept UTL_ATTRIBUTE(LIFETIMEBOUND) -> enable_if_t<!I, T const&&> {
        return move(head);
    }
...
struct storage<T, Tail...> : variadic_traits<T, Tail...> {
    ...
    template<typename UHead, typename... UTail, enable_if_t<
    ...
    ) : head(forward<UHead>(other_head)), tail(forward<UTail>(other_tail)...) {}
    ...
    constexpr storage(allocator_arg_t, Alloc const& alloc) noexcept(
        conjunction<
            UTL_SCOPE is_nothrow_constructible<T, allocator_arg_t, Alloc const&>,
            UTL_SCOPE is_nothrow_constructible<tail_type, allocator_arg_t, Alloc const&>
        >::value
    ) : head(allocator_arg, alloc), tail(allocator_arg, alloc) {}
    ...
    constexpr storage(allocator_arg_t, Alloc const& alloc) noexcept(
        conjunction<
            UTL_SCOPE is_nothrow_constructible<T, Alloc const&>,
            UTL_SCOPE is_nothrow_constructible<tail_type, allocator_arg_t, Alloc const&>
        >::value
    ) : head(alloc), tail(allocator_arg, alloc) {}
    ...
    constexpr storage(allocator_arg_t, Alloc const& alloc) noexcept(
        conjunction<
            UTL_SCOPE is_nothrow_constructible<T>,
            UTL_SCOPE is_nothrow_constructible<tail_type, allocator_arg_t, Alloc const&>
        >::value
    ) : head(), tail(allocator_arg, alloc) {}

    template<typename Alloc, typename UHead, typename... UTail, enable_if_t<
        conjunction<
            UTL_SCOPE uses_allocator<T, Alloc>,
            UTL_SCOPE is_constructible<T, allocator_arg_t, Alloc const&, UHead>
        >::value,
    int> = 0>
    constexpr storage(allocator_arg_t, Alloc const& alloc, UHead&& other_head, UTail&&... other_tail) noexcept(
        conjunction<
            UTL_SCOPE is_nothrow_constructible<T, allocator_arg_t, Alloc const&, UHead>,
            UTL_SCOPE is_nothrow_constructible<tail_type, allocator_arg_t, Alloc const&, UTail...>
        >::value
    ) : head(allocator_arg, alloc, forward<UHead>(other_head)), tail(allocator_arg, alloc, forward<UTail>(other_tail)...) {}

    template<typename Alloc, typename UHead, typename... UTail, enable_if_t<
        conjunction<
            UTL_SCOPE uses_allocator<T, Alloc>,
            negation<UTL_SCOPE is_constructible<T, allocator_arg_t, Alloc const&>>,
            UTL_SCOPE is_constructible<T, UHead, Alloc const&>
        >::value,
    int> = 1>
    constexpr storage(allocator_arg_t, Alloc const& alloc, UHead&& other_head, UTail&&... other_tail) noexcept(
        conjunction<
            UTL_SCOPE is_nothrow_constructible<T, UHead, Alloc const&>,
            UTL_SCOPE is_nothrow_constructible<tail_type, allocator_arg_t, Alloc const&, UTail...>
        >::value
    ) : head(forward<UHead>(other_head), alloc), tail(allocator_arg, alloc, forward<UTail>(other_tail)...) {}
    ...
    constexpr storage(allocator_arg_t, Alloc const& alloc, UHead&& other_head, UTail&&... other_tail) noexcept(
        conjunction<
            ...
            UTL_SCOPE is_nothrow_constructible<tail_type, allocator_arg_t, Alloc const&, UTail...>
        >::value
    ) : head(forward<UHead>(other_head)), tail(allocator_arg, alloc, forward<UTail>(other_tail)...) {}
    ...
    template<typename... Us>
    UTL_ATTRIBUTE(ALWAYS_INLINE)
    UTL_CONSTEXPR_CXX14 void swap(storage<Us...> const& other) const noexcept(
        traits::template is_nothrow_const_swappable_with<Us const&...>::value
    ) {
        UTL_SCOPE utility::swap(head, other.head);
        tail.swap(other.tail);
    }
    ...
    template<typename... Us>
    UTL_ATTRIBUTE(ALWAYS_INLINE)
    UTL_CONSTEXPR_CXX14 void swap(storage<Us...>& other) const noexcept(
        traits::template is_nothrow_const_swappable_with<Us&...>::value
    ) {
        UTL_SCOPE utility::swap(head, other.head);
        tail.swap(other.tail);
    }
    ...
    template<typename UHead, typename... UTail>
    UTL_ATTRIBUTE(ALWAYS_INLINE)
    constexpr storage const& assign(UHead&& other_head, UTail&&... other_tail) const noexcept(
        traits::template is_nothrow_const_assignable<UHead, UTail...>::value
    ) {
        head = forward<UHead>(other_head);
        tail.assign(forward<UTail>(other_tail)...);
        return *this;
    ...
    template<size_t I>
    UTL_ATTRIBUTES(NODISCARD, CONST, FLATTEN) UTL_CONSTEXPR_CXX14 auto get() && noexcept UTL_ATTRIBUTE(LIFETIMEBOUND) -> result_type_t<I>&& {
    ...
    template<size_t I>
    UTL_ATTRIBUTES(NODISCARD, CONST, FLATTEN) constexpr auto get() const && noexcept UTL_ATTRIBUTE(LIFETIMEBOUND) -> result_type_t<I> const&& {
        return move(tail).template get<I - 1>();
    }
...
struct offset_impl<I, storage<T0, Ts...>, enable_if_t<(I > 0)>> {
    ...
    static constexpr size_t value = offset_impl<I - 1, storage<Ts...>>::value + offsetof(type, tail);
};
...
private:
    ...
    using swap_type = conditional_t<traits::is_swappable, tuple, details::tuple::invalid_swap_t<Types...>>;
    using const_swap_type = conditional_t<traits::is_const_swappable, tuple, details::tuple::invalid_swap_t<Types...>> const;
    using move_assign_t = conditional_t<traits::is_move_assignable,
        details::tuple::invalid_t,
        tuple>;
    using move_construct_t = conditional_t<traits::is_move_constructible,
        details::tuple::invalid_t,
        tuple>;
    ...
    constexpr tuple(TupleLike&& other, index_sequence<Is...>) noexcept(
        conjunction<
            TT_SCOPE is_all_nothrow_gettable<TupleLike, sizeof...(Is)>,
            TT_SCOPE rebind_references_t<traits::template is_nothrow_constructible, TupleLike, sizeof...(Is)>
        >::value
    ) : base_type(TT_SCOPE get<Is>(forward<TupleLike>(other))...) {}

    template<typename Alloc, typename TupleLike, size_t... Is>
    constexpr tuple(allocator_arg_t, Alloc const& alloc, TupleLike&& other, index_sequence<Is...>) noexcept(
        conjunction<
            TT_SCOPE is_all_nothrow_gettable<TupleLike, sizeof...(Is)>,
            TT_SCOPE rebind_references_t<
                variadic_proxy<
                    traits::template is_nothrow_constructible_with_allocator,
                    Alloc
                >::template apply,
                TupleLike,
                sizeof...(Types)
    ...
    UTL_CONSTEXPR_CXX14 tuple& assign(TupleLike&& other, index_sequence<Is...>) noexcept(
        conjunction<
            TT_SCOPE is_all_nothrow_gettable<TupleLike, sizeof...(Is)>,
            TT_SCOPE rebind_references_t<traits::template is_nothrow_assignable, TupleLike, sizeof...(Is)>
        >::value
    ) {
    ...
    template<typename TupleLike, size_t... Is>
    constexpr tuple const& assign(TupleLike&& other, index_sequence<Is...>) const noexcept(
        conjunction<
            TT_SCOPE is_all_nothrow_gettable<TupleLike, sizeof...(Is)>,
            TT_SCOPE rebind_references_t<traits::template is_nothrow_const_assignable, TupleLike, sizeof...(Is)>
        >::value
    ) {
...
public:
    ...
    UTL_CONSTEXPR_CXX14 tuple& operator=(tuple const& other) noexcept(traits::is_nothrow_copy_assignable) = default;
    ...
    template<bool NotEmpty = (sizeof...(Types) >= 1), typename = enable_if_t<
        NotEmpty &&
        traits::is_const_copy_assignable
    >>
    constexpr tuple const& operator=(tuple const& other) const noexcept(traits::is_nothrow_const_copy_assignable) {
        return assign(other, index_sequence_for<Types...>{});
    }

    template<bool NotEmpty = (sizeof...(Types) >= 1), typename = enable_if_t<
        NotEmpty &&
        traits::is_const_move_assignable
    >>
    constexpr tuple const& operator=(tuple&& other) const noexcept(traits::is_nothrow_const_move_assignable) {
...
public:
    ...
    UTL_CONSTEXPR_CXX14 enable_if_t<
        conjunction<
            typename traits::template matches<Us...>,
            typename traits::template is_swappable_with<Us&...>
        >::value
    > swap(tuple<Us...>& other) noexcept(
        traits::template is_nothrow_swappable_with<Us&...>::value
    ) {
        static_assert(is_base_of<details::tuple::storage<Us...>, tuple<Us...>>::value, "tuple must inherit from storage");
    ...
    UTL_CONSTEXPR_CXX14 enable_if_t<
        conjunction<
            typename traits::template matches<Us...>,
            typename traits::template is_swappable_with<Us const&...>
        >::value
...
public:
    template<bool NotEmpty = (sizeof...(Types) >= 1), enable_if_t<
        NotEmpty &&
        traits::is_implicit_default_constructible,
    int> = 0>
    ...
    template<bool NotEmpty = (sizeof...(Types) >= 1), enable_if_t<
        NotEmpty &&
        traits::is_explicit_default_constructible,
    int> = 1>
...
public:
    template<bool NotEmpty = (sizeof...(Types) >= 1), enable_if_t<
        conjunction<
            bool_constant<NotEmpty>,
            typename traits::template is_implicit_constructible<Types const&...>
        >::value,
    int> = 0>
    constexpr tuple(Types const&... args) noexcept(traits::is_nothrow_copy_constructible) : base_type(args...) {}

    template<bool NotEmpty = (sizeof...(Types) >= 1), enable_if_t<
        conjunction<
            bool_constant<NotEmpty>,
            typename traits::template is_explicit_constructible<Types const&...>
        >::value,
    int> = 1>
    explicit constexpr tuple(Types const&... args) noexcept(traits::is_nothrow_copy_constructible) : base_type(args...) {}

public:
    template<typename UHead, typename... UTail, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTail) + 1>,
            typename traits::template is_implicit_constructible<UHead, UTail...>,
            negation<typename traits::template is_dangling<UHead&&, UTail&&...>>
        >::value,
    int> = 0>
    constexpr tuple(UHead&& head, UTail&&... tail) noexcept(conjunction<
        bool_constant<sizeof...(Types) == sizeof...(UTail) + 1>,
        typename traits::template is_nothrow_constructible<UHead, UTail...>
    >::value) : base_type(forward<UHead>(head), forward<UTail>(tail)...) {}
    ...
    template<typename UHead, typename... UTail, enable_if_t<
        ...
        >::value,
    int> = 1>
    explicit constexpr tuple(UHead&& head, UTail&&... tail) noexcept(traits::template is_nothrow_constructible<UHead, UTail...>::value)
    ...
    template<typename UHead, typename... UTail, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTail) + 1>,
            typename traits::template is_implicit_constructible<UHead, UTail...>,
            typename traits::template is_dangling<UHead&&, UTail&&...>
        >::value,
    int> = 2>
    constexpr tuple(UHead&& head, UTail&&... tail) noexcept(traits::template is_nothrow_constructible<UHead, UTail...>::value) = delete;
    ...
    template<typename UHead, typename... UTail, enable_if_t<
        ...
        >::value,
    int> = 3>
    explicit constexpr tuple(UHead&& head, UTail&&... tail) noexcept(traits::template is_nothrow_constructible<UHead, UTail...>::value) = delete;
...
public:
    template<typename... UTypes, enable_if_t<
        conjunction<
            ...
            negation<typename traits::template is_dangling<UTypes&...>>
        >::value,
    int> = 0>
    ...
    template<typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_implicit_constructible<UTypes&...>,
            typename traits::template is_dangling<UTypes&...>
        >::value,
    int> = 2>
    constexpr tuple(tuple<UTypes...>& other) noexcept(
        traits::template is_nothrow_constructible<UTypes&...>::value
...
public:
    template<typename... UTypes, enable_if_t<
        conjunction<
            ...
            negation<typename traits::template is_dangling<UTypes const&...>>
        >::value,
    int> = 0>
    ...
    template<typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_implicit_constructible<UTypes const&...>,
            typename traits::template is_dangling<UTypes const&...>
        >::value,
    int> = 2>
...
public:
    ...
    template<typename... UTypes, enable_if_t<
        disjunction<
            negation<typename traits::template is_constructible<UTypes&&...>>
        >::value,
    int> = 4>
...
public:
    ...
    template<typename... UTypes, enable_if_t<
        disjunction<
            negation<typename traits::template is_constructible<UTypes const&&...>>
        >::value,
    int> = 4>
...
public:
    template<typename TupleLike, enable_if_t<
        conjunction<
            not_this<TupleLike>,
            TT_SCOPE is_all_gettable<TupleLike, sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_implicit_constructible, TupleLike, sizeof...(Types)>,
            negation<TT_SCOPE rebind_references_t<traits::template is_dangling, TupleLike, sizeof...(Types)>>
        >::value,
    int> = 0>
    constexpr tuple(TupleLike&& p) noexcept(
        conjunction<
            TT_SCOPE is_all_nothrow_gettable<TupleLike, sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_nothrow_constructible, TupleLike, sizeof...(Types)>
        >::value
    ...
    template<typename TupleLike, enable_if_t<
        conjunction<
            not_this<TupleLike>,
            TT_SCOPE is_all_gettable<TupleLike, sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_explicit_constructible, TupleLike, sizeof...(Types)>,
            negation<TT_SCOPE rebind_references_t<traits::template is_dangling, TupleLike, sizeof...(Types)>>
        >::value,
    int> = 1>
    ...
    template<typename TupleLike, enable_if_t<
        conjunction<
            ...
            TT_SCOPE is_all_gettable<TupleLike, sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_implicit_constructible, TupleLike, sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_dangling, TupleLike, sizeof...(Types)>
        >::value,
    int> = 2>
    constexpr tuple(TupleLike&& p) noexcept(
        conjunction<
...
public:
    template<typename Alloc, enable_if_t<
        traits::template is_implicit_constructible_with_allocator<Alloc>::value,
    int> = 0>
    ...
    template<typename Alloc, enable_if_t<
        traits::template is_explicit_constructible_with_allocator<Alloc>::value,
    int> = 1>
...
public:
    template<typename Alloc, enable_if_t<
        traits::template is_implicit_constructible_with_allocator<Alloc, Types const&...>::value,
    int> = 0>
    ...
    template<typename Alloc, enable_if_t<
        traits::template is_explicit_constructible_with_allocator<Alloc, Types const&...>::value,
    int> = 1>
...
public:
    template<typename Alloc, typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_implicit_constructible_with_allocator<Alloc, UTypes...>,
            negation<typename traits::template is_dangling_without_allocator<Alloc, UTypes...>>
        >::value,
    int> = 0>
    ...
    template<typename Alloc, typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_explicit_constructible_with_allocator<Alloc, UTypes...>,
            negation<typename traits::template is_dangling_without_allocator<Alloc, UTypes...>>
        >::value,
    int> = 1>
    ...
    template<typename Alloc, typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_implicit_constructible_with_allocator<Alloc, UTypes...>,
            typename traits::template is_dangling_without_allocator<Alloc, UTypes...>
        >::value,
    int> = 2>
    constexpr tuple(allocator_arg_t, Alloc const& alloc, UTypes&&... args) noexcept(
        traits::template is_nothrow_constructible_with_allocator<Alloc, UTypes...>::value
...
public:
    template<typename Alloc, typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_implicit_constructible_with_allocator<Alloc, UTypes const&...>,
            negation<typename traits::template is_dangling_without_allocator<Alloc, UTypes const&...>>
        >::value,
    int> = 0>
    ...
    template<typename Alloc, typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_explicit_constructible_with_allocator<Alloc, UTypes const&...>,
            negation<typename traits::template is_dangling_without_allocator<Alloc, UTypes const&...>>
        >::value,
    int> = 1>
    explicit constexpr tuple(allocator_arg_t, Alloc const& alloc, tuple<UTypes...> const& other) noexcept(
        traits::template is_nothrow_constructible_with_allocator<Alloc, UTypes const&...>::value
    ) : tuple(allocator_arg, alloc, other, index_sequence_for<UTypes...>{}) {}
    ...
    template<typename Alloc, typename... UTypes, enable_if_t<
        conjunction<
            bool_constant<sizeof...(Types) == sizeof...(UTypes)>,
            typename traits::template is_implicit_constructible_with_allocator<Alloc, UTypes const&...>,
            typename traits::template is_dangling_without_allocator<Alloc, UTypes const&...>
        >::value,
    int> = 2>
    constexpr tuple(allocator_arg_t, Alloc const& alloc, tuple<UTypes...> const& other) noexcept(
        traits::template is_nothrow_constructible_with_allocator<Alloc, UTypes const&...>::value
...
public:
    ...
    template<typename Alloc, typename... UTypes, enable_if_t<
        disjunction<
            negation<typename traits::template is_constructible_with_allocator<Alloc, UTypes&&...>>
        >::value,
    int> = 4>
...
public:
    ...
    template<typename Alloc, typename... UTypes, enable_if_t<
        disjunction<
            negation<typename traits::template is_constructible_with_allocator<Alloc, UTypes const&&...>>
        >::value,
    int> = 4>
...
public:
    template<typename Alloc, typename TupleLike, enable_if_t<
        conjunction<
            TT_SCOPE is_tuple_like<TupleLike>,
            not_this<TupleLike>,
            bool_constant<tuple_size<TupleLike>::value == sizeof...(Types)>,
            TT_SCOPE rebind_references_t<
                variadic_proxy<
                    traits::template is_implicit_constructible_with_allocator,
                    Alloc
                >::template apply,
                TupleLike
            >,
            negation<
                TT_SCOPE rebind_references_t<
                    variadic_proxy<
                        traits::template is_dangling_without_allocator,
                        Alloc
                    >::template apply,
                    TupleLike
                >
...
public:
    ...
    template<typename Alloc, typename = enable_if_t<
        traits::template is_constructible_with_allocator<Alloc, Types&&...>::value
    >>
...
public:
    template<typename... UTypes, typename = enable_if_t<
        conjunction<
            typename traits::template is_assignable<UTypes const&...>
        >::value
    >>
    ...
    template<typename... UTypes, typename = enable_if_t<
        conjunction<
            typename traits::template is_const_assignable<UTypes const&...>
        >::value
    >>
    constexpr tuple const& operator=(tuple<UTypes...> const& other) const noexcept(
        traits::template is_nothrow_const_assignable<UTypes const&...>::value
    ) {
        return assign(other, index_sequence_for<Types...>{});
    }
...
public:
    template<typename... UTypes, enable_if_t<
        conjunction<
            typename traits::template is_assignable<UTypes&&...>
        >::value,
    int> = 0>
    ...
    template<typename... UTypes, enable_if_t<
        disjunction<
            negation<typename traits::template is_assignable<UTypes&&...>>
        >::value,
    int> = 1>
    ...
    template<typename... UTypes, enable_if_t<
        conjunction<
            typename traits::template is_const_assignable<UTypes&&...>
        >::value,
    int> = 0>
    constexpr tuple const& operator=(tuple<UTypes...>&& other) const noexcept(
        traits::template is_nothrow_const_assignable<UTypes&&...>::value
    ) {
        return assign(move(other), index_sequence_for<Types...>{});
    }
    ...
    template<typename... UTypes, enable_if_t<
        disjunction<
            negation<typename traits::template is_const_assignable<UTypes&&...>>
        >::value,
    int> = 1>
...
public:
    template<typename TupleLike, enable_if_t<
        conjunction<
            ...
            not_this<TupleLike>,
            bool_constant<tuple_size<TupleLike>::value == sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_assignable, TupleLike, sizeof...(Types)>
        >::value,
    int> = 0>
    UTL_CONSTEXPR_CXX14 tuple& operator=(TupleLike&& other) noexcept(
        conjunction<
            TT_SCOPE is_all_nothrow_gettable<TupleLike, sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_nothrow_assignable, TupleLike, sizeof...(Types)>
        >::value
    ) {
    ...
    template<typename TupleLike, enable_if_t<
        conjunction<
            ...
            not_this<TupleLike>,
            bool_constant<tuple_size<TupleLike>::value == sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_const_assignable, TupleLike, sizeof...(Types)>
        >::value,
    int> = 1>
    constexpr tuple const& operator=(TupleLike&& other) const noexcept(
        conjunction<
            TT_SCOPE is_all_nothrow_gettable<TupleLike, sizeof...(Types)>,
            TT_SCOPE rebind_references_t<traits::template is_nothrow_const_assignable, TupleLike, sizeof...(Types)>
        >::value
    ) {
        return assign(forward<TupleLike>(other), index_sequence_for<Types...>{});
    }
...
struct tuple_element_offset<I, tuple<Ts...>> : enable_if_t<
    is_base_of<details::tuple::storage<Ts...>, tuple<Ts...>>::value,
...
UTL_ATTRIBUTES(NODISCARD, CONST) constexpr enable_if_t<
    (I == tuple_size<T>::value),
bool> less(T const& l, U const& r) noexcept {
    return false;
}
...
UTL_ATTRIBUTES(NODISCARD, CONST) constexpr enable_if_t<
    (I < tuple_size<T>::value),
bool> less(T const& l, U const& r) noexcept(conjunction<
    compare_ops::has_nothrow_eq<T, U>,
    compare_ops::has_nothrow_lt<T, U>
>::value
) {
    return (TT_SCOPE get<I - 1>(l) == TT_SCOPE get<I - 1>(r)) && ((TT_SCOPE get<I>(l) < TT_SCOPE get<I>(r)) || less<I + 1>(l, r));
}

template<typename T, typename U>
UTL_ATTRIBUTES(NODISCARD, FLATTEN)
constexpr bool less(T const& l, U const& r) noexcept(conjunction<
    compare_ops::has_nothrow_eq<T, U>,
    compare_ops::has_nothrow_lt<T, U>
>::value
) {
...
UTL_ATTRIBUTES(NODISCARD, CONST) constexpr enable_if_t<
    (I == tuple_size<T>::value),
bool> equals(T const& l, U const& r) noexcept {
    return true;
}
...
UTL_NODISCARD constexpr enable_if_t<conjunction<
        compare_ops::all_have_eq<tuple<Ts...>, tuple<Us...>>
    >::value,
bool> operator==(tuple<Ts...> const& l, tuple<Us...> const& r) noexcept(details::tuple::equals(l, r)) {
...
UTL_NODISCARD constexpr enable_if_t<conjunction<
        compare_ops::all_have_eq<tuple<Ts...>, tuple<Us...>>,
        compare_ops::all_have_lt<tuple<Ts...>, tuple<Us...>>
    >::value,
bool> operator<(tuple<Ts...> const& l, tuple<Us...> const& r) noexcept(details::tuple::less(l, r)) {
...
UTL_NODISCARD constexpr enable_if_t<
    compare_ops::has_eq<tuple<Ts...>,tuple<Us...>>::value,
bool> operator!=(tuple<Ts...> const& l, tuple<Us...> const& r) noexcept(
...
template<typename... Ts, typename... Us>
UTL_NODISCARD constexpr enable_if_t<
    compare_ops::has_lt<tuple<Us...>, tuple<Ts...>>::value,
bool> operator<=(tuple<Ts...> const& l, tuple<Us...> const& r) noexcept(
...
template<typename... Ts, typename... Us>
UTL_NODISCARD constexpr enable_if_t<
    compare_ops::has_lt<tuple<Ts...>, tuple<Us...>>::value,
bool> operator>=(tuple<Ts...> const& l, tuple<Us...> const& r) noexcept(
...
UTL_NODISCARD constexpr
tuple<Args&...> tie(Args&... args UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept {
    return {args...};
}

template <typename... Args>
UTL_NODISCARD constexpr
tuple<Args&&...> forward_as_tuple(Args&&... args UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept {
