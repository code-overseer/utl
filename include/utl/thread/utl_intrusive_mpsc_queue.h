...
protected:
    static constexpr struct non_block_t {} non_block = {};
private:
    ...
    pointer* next_ptr(reference node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept {
        return addressof(get_next_ref(node));
    }
    static constexpr
    UTL_ATTRIBUTES(CONST, NODISCARD)
    pointer* next_ptr(sentinel_t& sentinel UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept {
        return addressof(get_next_ref(sentinel));
    }
    static constexpr
    UTL_ATTRIBUTES(NODISCARD)
    pointer* next_ptr(const_reference node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept {
        return next_ptr(const_cast<reference>(node));
    }
    static constexpr 
    UTL_ATTRIBUTES(CONST, NODISCARD)
    pointer* next_ptr(sentinel_t const& sentinel UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept {
        return next_ptr(const_cast<sentinel_t&>(sentinel));
    }
protected:

    mpsc_queue_base() noexcept : sentinel_(), head_(&sentinel_), tail_(&sentinel_) {}
    mpsc_queue_base(const mpsc_queue_base&) noexcept = delete;
    mpsc_queue_base& operator=(const mpsc_queue_base&) noexcept = delete;
    ...
    enable_if<is_dequeue_param<T>::value, pointer> dequeue_impl(T parameter) noexcept {
        ...
        if (next_head == nullptr) {
            // next_head is nullptr, so we have just arrived at tail_; i.e. output == tail_ when we loaded next_head
            if ((void*)output != tail_.load(std::memory_order_relaxed)) {
                // In this branch, someone is in the process of enqueuing,
                // the tail_ exchange has occured, but potentially the next pointer of output hasn't been set
                // Just exit, and let the caller call again
                return nullptr;
            }
...
private:
    ...
    bool is_sentinel(const_pointer node) const noexcept {
        return (const void*)node == (const void*)addressof(sentinel_);
    }
 ...
 * Implementation is based on https://www.1024cores.net/home/lock-free-algorithms/queues/intrusive-mpsc-node-based-queue
 ...
 * * Declare a public non-static function member with the following signatures `std::atomic<NodeType*>& Getnext_node()`
 * * Otherwise, the expression `Getnext_node(node)` returning `std::atomic<NodeType*>&` must be valid through argument-dependent-lookup, where
 * `node` is of type `NodeType*`
 ...
 *
 *  **
 *  * Retrieves/creates a pointer to a node from any set of arguments that will be passed from `mspc_queue::enqueue`
 *  *
...
public:
     ...
     * 
     * As the enqueue operation is not atomic, if a dequeue of the last node in the queue races with the enqueue
     * operation, a safety mechanism will force a nullptr to be returned instead
     *
     * @return
     * A valid node_pointer will be returned if the queue is non-empty and there was no enqueue race-condition
     * with the final node; otherwise, nullptr will be returned
     */
    node_pointer dequeue() noexcept(
        is_nothrow_constructible_v<node_deleter, node_policy const&> &&
        noexcept(node_pointer(dequeue_impl(non_block), decl_deleter()))
    ) {
     ...
     * 
     * The returned node pointer uses a copy of the node policy implementation of the current queue instance
     *
     * @return 
     * If the calling thread was notified, a valid node_pointer will be returned if the queue is non-empty
     * and there was no enqueue race-condition with the final node; otherwise, nullptr will be returned
     */
    template<typename R, typename P>
    node_pointer dequeue_wait_for(chrono::duration<R,P> const& timeout) noexcept(
        is_nothrow_constructible_v<node_deleter, node_policy const&> &&
        noexcept(node_pointer(dequeue_impl(decl_duration<R,P>()), decl_deleter()))
    ) {
        return node_pointer(dequeue_impl(timeout), node_deleter(static_cast<node_policy const&>(*this)));
    }
     ...
     */
    void clear() noexcept(
        noexcept(node_policy::release((node_type*)0)) &&
        noexcept(dequeue_impl())
    ) {
