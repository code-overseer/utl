...
template<typename T>
struct add_pointer { using type = UTL_BUILTIN_add_pointer(T); };
...
template<typename T>
auto add_ptr(float) noexcept -> decltype(static_cast<T*(*)(T const volatile*)>(0)((void const volatile*)0));
template<typename T>
T add_ptr(...) noexcept;
}
}
...
template<typename T>
struct add_pointer { using type = decltype(details::type_traits::add_ptr<T>(0)); };
...
template<typename T>
struct remove_pointer { using type = UTL_BUILTIN_remove_pointer(T); };
