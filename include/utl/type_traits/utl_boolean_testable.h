...
template<typename T>
struct has_native_and<T, enable_if_t<
        conjunction<
            is_boolean<decltype(declval<T>() && true)>,
            is_boolean<decltype(true && declval<T>())>,
            bool_constant<!(false && declval<T>())> // if short-circuiting succeeds, declval is not invoked
        >::value
    >
> : true_type {};

template<typename T>
struct conjunctable : conjunction<
    no_adl_and<T>,
    no_member_and<T>,
    has_native_and<T>
> {};
...
template<typename T>
struct has_native_or<T, enable_if_t<
        conjunction<
            is_boolean<decltype(declval<T>() || false)>,
            is_boolean<decltype(false || declval<T>())>,
            bool_constant<(true || declval<T>())> // if short-circuiting succeeds, declval is not invoked
        >::value
    >
> : true_type {};

template<typename T>
struct disjunctable : conjunction<
    no_adl_or<T>,
    no_member_or<T>,
    has_native_or<T>
> {};
...
template<typename T>
struct is_boolean_testable : conjunction<
    details::bool_testable::castable<T, bool>,
    details::bool_testable::negatable<T>,
    details::bool_testable::conjunctable<T>,
    details::bool_testable::disjunctable<T>
> {};
