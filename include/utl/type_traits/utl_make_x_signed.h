...
template<typename T>
struct make_signed { using type = UTL_BUILTIN_make_signed(T); };
...
template<> struct signed_table_entry<N> { using type = T; }
...
struct find_signed<T, Idx, true> : conditional_t<sizeof(T) == sizeof(signed_table_entry_t<Idx>),
    signed_table_entry<Idx>,
    find_signed<T, Idx + 1>
> {};
}
}
...
template<typename T>
struct make_signed { using type = details::type_traits::find_signed_t<T>; };
template<typename T>
struct make_signed<T const> { using type = typename make_signed<T>::type const; };
template<typename T>
struct make_signed<T volatile> { using type = typename make_signed<T>::type volatile; };
template<typename T>
struct make_signed<T const volatile> { using type = typename make_signed<T>::type const volatile; };
template<>
...
template<>
struct make_signed<signed long> { using type = signed long; };
...
#define UTL_TRAIT_SUPPORTED_make_signed UTL_TRAIT_SUPPORTED_is_integral && UTL_TRAIT_SUPPORTED_is_enum
...
template<typename T>
struct make_unsigned { using type = UTL_BUILTIN_make_unsigned(T); };
...
#define UTL_GENERATE_TABLE_ENTRY(N, T) \
template<> struct unsigned_table_entry<N> { using type = T; }
...
template<typename T, size_t Idx>
struct find_unsigned<T, Idx, true> : conditional_t<sizeof(T) == sizeof(unsigned_table_entry_t<Idx>),
    unsigned_table_entry<Idx>,
...
template<typename T>
struct make_unsigned { using type = details::type_traits::find_unsigned_t<T>; };
template<typename T>
struct make_unsigned<T const> { using type = typename make_unsigned<T>::type const; };
template<typename T>
struct make_unsigned<T volatile> { using type = typename make_unsigned<T>::type volatile; };
template<typename T>
struct make_unsigned<T const volatile> { using type = typename make_unsigned<T>::type const volatile; };
template<>
...
#define UTL_TRAIT_SUPPORTED_make_unsigned UTL_TRAIT_SUPPORTED_is_integral && UTL_TRAIT_SUPPORTED_is_enum
