...
auto implicit_test(float) -> decltype((declval<void(T)>()({declval<THead>(), declval<TTail>()...}), is_constructible<T, THead, TTail...>{}));
...
auto explicit_test(int) -> decltype((declval<void(T)>()({declval<THead>(), declval<TMid>(), declval<TTail>()...}), false_type{}));
template<typename T, typename THead, typename TMid, typename... TTail>
auto explicit_test(float) -> decltype((declval<void(T)>()(T{declval<THead>(), declval<TMid>(), declval<TTail>()...}), is_constructible<T, THead, TMid, TTail...>{}));
template<typename T, typename THead>
auto explicit_test(int) -> conjunction<negation<is_convertible<THead, T>>, is_constructible<T, THead>>;
template<typename T>
auto explicit_test(int) -> conjunction<
    negation<decltype(implicit_test<T>(0))>,
...
template <typename TTarget, typename... TArgs>
struct is_nothrow_implicit_constructible : conjunction<is_implicit_constructible<TTarget, TArgs...>, is_nothrow_constructible<TTarget, TArgs...>> {};

template <typename TTarget, typename... TArgs>
struct is_explicit_constructible : constructible::details::is_explicit<TTarget, TArgs...> {};

template <typename TTarget, typename... TArgs>
struct is_nothrow_explicit_constructible : conjunction<is_explicit_constructible<TTarget, TArgs...>, is_nothrow_constructible<TTarget, TArgs...>> {};
