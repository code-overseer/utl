...
template<typename T0, typename T1>
struct simple_common_ref<T0&&, T1&&, enable_if_t<
        all_convertible_to<rvalue_ref_t<simple_common_ref_t<T0&, T1&>>, T0&&, T1&&>::value
    >   // enable_if_t
> : rvalue_ref<simple_common_ref_t<T0&, T1&>> {};

template<typename T0, typename T1>
struct simple_common_ref<T0&, T1&&, enable_if_t<
        is_convertible<T1&&, simple_common_ref_t<T0&, add_const_t<T1>&>>::value
    >   // enable_if_t
> : simple_common_ref<T0&, add_const_t<T1>&> {};

template<typename T0, typename T1>
struct simple_common_ref<T0&&, T1&, void_t<
        simple_common_ref_t<T1&, T0&&>
    >   // void_t
...
using impl = first_type<
    simple_common_ref<T, U>,
    basic_common_reference<
        remove_cvref_t<T>,
        remove_cvref_t<U>,
        copy_cvref_from<T>::template apply,
        copy_cvref_from<U>::template apply
    >, // basic_common_reference
    ternary_result<T, U>,
    common_type<T, U>
>;
