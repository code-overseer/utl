...
template<typename T>
struct remove_extent { using type = UTL_BUILTIN_remove_extent(T); };
...
template<typename T>
struct remove_all_extent { using type = T; };
template<typename T>
using remove_all_extent_t = typename remove_all_extent<T>::type;

template<typename T>
struct remove_all_extent<T[]> { using type = remove_all_extent_t<T>; };
template<typename T, size_t N>
struct remove_all_extent<T[N]> { using type = remove_all_extent_t<T>; };
