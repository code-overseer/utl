...
private:
    ...
    friend constexpr
    void next(value_type& node, pointer_type value) noexcept { node.next_ = value; }
    friend constexpr
    pointer_type next(const_reference_type node) noexcept { return node->next_; }
    friend constexpr
    UTL_ATTRIBUTE(CONST)
    pointer_type& next_ref(reference_type node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept { return node->next_; }
};
...
private:
    ...
    UTL_ATTRIBUTE(CONST)
    friend constexpr pointer_type& next_ref(reference_type node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept { return node->next_; }
    ...
    UTL_ATTRIBUTE(CONST)
    friend constexpr pointer_type& prev_ref(reference_type node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept { return node->prev_; }
};
...
template <next_node_container NodeType>
UTL_ATTRIBUTE(CONST)
constexpr NodeType*& next_ref(NodeType& node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept { return node.next; }
template <next_node_referenceable NodeType> requires (!next_node_container<NodeType*>)
UTL_ATTRIBUTE(CONST)
constexpr NodeType*& next_ref(NodeType& node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept { return node.next_ref(); }
...
template <prev_node_container NodeType>
UTL_ATTRIBUTE(CONST)
constexpr NodeType*& prev_ref(NodeType& node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept { return node.prev; }
template <prev_node_referenceable NodeType> requires (!prev_node_container<NodeType*>)
UTL_ATTRIBUTE(CONST)
constexpr NodeType*& prev_ref(NodeType& node UTL_ATTRIBUTE(LIFETIMEBOUND)) noexcept { return node.prev_ref(); }
...
concept unidirectional =
    ...
    requires(NodeType& node) { {next_ref(node)} -> same_as<NodeType*&>; };
...
struct get_next_t {
    ...
    constexpr decltype(auto) operator()(const NodeType& node) const noexcept {
        return next(node);
    }
};
...
struct get_next_ref_t {
    template<unidirectional NodeType> requires (
        !is_base_of_v<unidirectional_node<NodeType>, NodeType> &&
        !is_base_of_v<bidirectional_node<NodeType>, NodeType>
    )
    constexpr NodeType& operator()(const NodeType& node UTL_ATTRIBUTE(LIFETIMEBOUND)) const noexcept {
        return next_ref(node);
    }

    template<typename NodeType> requires (
        is_base_of_v<unidirectional_node<NodeType>, NodeType> ||
        is_base_of_v<bidirectional_node<NodeType>, NodeType>
    )
    UTL_ATTRIBUTE(CONST)
...
concept bidirectional = unidirectional<NodeType> &&
    ...
    requires(const NodeType& node) { { prev_ref(node) } -> same_as<NodeType*&>; };
...
struct get_prev_ref_t {
    template<bidirectional NodeType> requires (!is_base_of_v<bidirectional_node<NodeType>, NodeType>)
    constexpr decltype(auto) operator()(const NodeType& node UTL_ATTRIBUTE(LIFETIMEBOUND)) const noexcept {
        return prev_ref(node);
    }

    template<typename NodeType> requires is_base_of_v<bidirectional_node<NodeType>, NodeType>
    UTL_ATTRIBUTE(CONST)
    constexpr NodeType& operator()(const NodeType& node UTL_ATTRIBUTE(LIFETIMEBOUND)) const noexcept {
        return prev_ref(node);
    }
...
struct linked_t {
    template<bidirectional NodeType>
    constexpr bool operator()(const NodeType& node) const noexcept {
        return get_next(node) != nullptr || get_prev(node) != nullptr;
    }

    template<unidirectional NodeType> requires (!bidirectional<NodeType>)
    constexpr bool operator()(const NodeType& node) const noexcept {
...
struct propagate_on_container_swap<
    Policy,
    void_t<typename Policy::propagate_on_container_swap>> : 
...
struct is_always_equal<
    Policy,
    void_t<typename Policy::is_always_equal>> : 
...
struct node_policy_traits {
    ...
    using fancy_pointer = UTL_SCOPE node_ptr<NodeType, default_node_deleter<node_type, policy_type>>;
    ...
    using propagate_on_container_copy_assignment = details::propagate_on_container_copy_assignment<policy_type>;
    using propagate_on_container_move_assignment = details::propagate_on_container_move_assignment<policy_type>;
    ...
    static_assert(is_always_equal::value || UTL_SCOPE is_nothrow_equality_comparable<policy_type>::value,
    ...
    static_assert(!propagate_on_container_swap::value ||
        UTL_SCOPE is_nothrow_swappable<policy_type>::value,
        "If propogation on swap is required, policy must be nothrow swappable");

    template<typename... Args> requires acquire_invocable<policy_type, node_type, Args...>
    ...
    template<same_as<node_type> T> requires clone_invocable<policy_type, node_type>
    ...
    policy_type> select_on_container_copy_construction(T const& p) noexcept(
    ...
    bool> equals(T const&, T const&) noexcept { return true; }

    template<typename T = policy_type>
    static constexpr enable_if_t<
        is_same<T, policy_type>::value && 
        !is_always_equal::value,
    bool> equals(T const& left, T const& right) noexcept { return left == right; }
};
