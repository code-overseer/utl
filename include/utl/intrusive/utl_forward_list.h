...
public:
    ...
    UTL_ATTRIBUTE(NODISCARD) UTL_CONSTEXPR_CXX14
    ...
    constexpr
    pointer get_head() const noexcept {
        return head_;
    }

    UTL_CONSTEXPR_CXX14
    pointer exchange_head(pointer node) noexcept {
        return exchange(head_, node);
    }

    UTL_CONSTEXPR_CXX14
    pointer set_head(pointer node) noexcept {
        return head_ = node;
    }
...
public:
    ...
    constexpr iterator_base(value_type* node) noexcept :
        node_(node), list_(nullptr) {}
    constexpr iterator_base(base<NodeType>* b) noexcept :
        node_(nullptr), list_(b) {}

    UTL_ATTRIBUTE(NODISCARD) constexpr pointer node() const noexcept { return node_; }
    UTL_ATTRIBUTE(NODISCARD) constexpr pointer next() const noexcept { return list_ ? list_.get_head() : get_next(*node_); }
...
private:
    ...
    iterator<NodeType> unconst() const noexcept {
...
private:
    ...
    class tag_t {};
    using copy_ctor_t = conditional_t<
        policy_traits::is_node_clonable::value, impl, invalid_t>;
    using copy_assign_t = conditional_t<
        policy_traits::is_node_clonable::value, impl, invalid_t>;

    /* Deliberately undefined, because it can't be used */
    impl(tag_t<invalid_t>, invalid_t const&) noexcept;
    UTL_CONSTEXPR_CXX14 impl& assign(tag_t<invalid_t>, invalid_t const&) noexcept { return *this; }
    ...
    static pointer clone_all(policy_type& policy, pointer src) {
        ...
        UTL_TRY {
            ...
            for (pointer prev_dst = head, src_node = get_next(src);
                src_node != nullptr;
    ...
    impl(tag_t<T>, T const& other, policy_type const& p = 
        policy_traits::select_on_container_copy_construction(other.policy_)) : base_type(), policy_(p) {
...
public:
    ...
    constexpr impl(copy_ctor_t const& other, policy_type const& p) : impl(tag_t<copy_ctor_t>{}, other, p) {}
    ...
    UTL_CONSTEXPR_CXX14 impl& operator=(const copy_assign_t& other) noexcept {
...
public:
    ...
    constexpr forward_list() noexcept(is_nothrow_default_constructible<node_policy>::value) = default;
    ...
    constexpr explicit forward_list(pointer head, node_policy const& policy = node_policy()) noexcept(
        noexcept(impl_type(head, policy))
    ...
    constexpr
    forward_list(T&& other, node_policy const& policy) noexcept(
        policy_traits::is_always_equal::value
    ) : impl_type(move(other), policy) {}

    template<typename T = forward_list, enable_if_t<
        is_same<T, forward_list>::value &&
        is_copy_constructible<forward_list>::value
    , bool> = true>
    forward_list(T const& other, node_policy const& policy)
        : impl_type(other, policy) {}
    ...
    UTL_CONSTEXPR_CXX14 reference emplace_front(Args&&... args) noexcept(
        noexcept(node_policy::acquire(declval<Args>()...))
    ) UTL_ATTRIBUTE(LIFETIMEBOUND) {
        return base_type::push_front(
            policy_traits::acquire(this->policy_, forward<Args>(args)...));
    }
    ...
    iterator emplace_after(const_iterator pos, Args&&... args) noexcept(
        noexcept(policy_traits::acquire(this->policy_, declval<Args>()...))
    ) UTL_ATTRIBUTE(LIFETIMEBOUND) {
    ...
    iterator insert_after(const_iterator pos, fancy_pointer node) noexcept(
        noexcept(policy_traits::acquire(this->policy_, node.release()))
    ) UTL_ATTRIBUTE(LIFETIMEBOUND) {
        UTL_ASSERT(node != nullptr);
        UTL_ASSERT(pos != end());
    ...
    UTL_ATTRIBUTE(NODISCARD)
    UTL_CONSTEXPR_CXX14 policy_type& get_policy() noexcept {
        return this->policy_;
    }
    ...
    UTL_ATTRIBUTE(NODISCARD)
    UTL_CONSTEXPR_CXX14 iterator before_begin() noexcept {
        return iterator(this);
    }
    UTL_ATTRIBUTE(NODISCARD)
    constexpr const_iterator before_cbegin() const noexcept {
    ...
    UTL_ATTRIBUTE(NODISCARD)
    UTL_CONSTEXPR_CXX14 iterator begin() noexcept {
    ...
    UTL_ATTRIBUTE(NODISCARD)
    UTL_CONSTEXPR_CXX14 iterator end() noexcept {
        return iterator();
    }
    UTL_ATTRIBUTE(NODISCARD)
    ...
    template<typename Comparer>
    void merge(forward_list&& other, Comparer comp) { merge(other, move(comp)); }
    ...
    void reverse() noexcept {
        base_type::set_head(base_type::reverse(base_type::get_head()));
    }

    size_type unique() {
        return unique(UTL_SCOPE equal_to<NodeType>{});
    }
    ...
    size_type unique(BinaryPredicate pred) {
        ...
        while (++next != last) {
            if (pred(*first, *next)) {
                to_destroy.splice_after(to_destroy.cbefore_begin(),
                    *this, first);
                ++removed;
            } else {
                first = next;
    ...
    void sort() {
        return sort(UTL_SCOPE less<NodeType>{});
    }
...
private:
    ...
    iterator splice_after_impl(const_iterator pos, forward_list&& other,
        const_iterator before, const_iterator last) noexcept {
